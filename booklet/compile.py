...
tex_dir = 'booklet/abstracts/tex/'
txt_dir = 'booklet/abstracts/txt/'
...
def format_authors(authors): #----------------------------------------------
    ...
    for i in range(n_auth):
        if  ',' in authors[i]:
            ...
            subnames = authors[i].split(' ')
        subnames = list(filter(None, subnames)) #remove empty strings
        if len(subnames) > 2:
            print('long name')
...
def txt2tex(file_name, verbose=False) :
    ...
    # READ .TXT FILE
    with open(txt_dir + file_name, 'r') as inputfile:
